...
unsigned long encDebounceMs = 3;
unsigned long swDebounceMs = 200;
/* USER CODE END PV */
...
void Set_LED_Brightness(int percent) {
    ...
    int pulse = (period + 1) * percent / 100;

    __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, pulse);
}

//function to run when there's a GPIO interrupt
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
    unsigned long now = HAL_GetTick();

    if (GPIO_Pin == ENC_CLK_Pin) {
        if ((now - lastEncTime) < encDebounceMs)
            return;

        lastEncTime = now;

        int dtState = HAL_GPIO_ReadPin(ENC_DT_GPIO_Port, ENC_DT_Pin);

        if (dtState == GPIO_PIN_SET) {
            Set_LED_Brightness(brightness + brightnessStep);
        }
        else {
            Set_LED_Brightness(brightness - brightnessStep);
        }
    ...
    if (GPIO_Pin == ENC_SW_Pin) {
        if ((now - lastSwTime) < swDebounceMs)
            return;
